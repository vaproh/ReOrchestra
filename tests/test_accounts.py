...
class TestAccountImport:
    ...
    def test_import_with_duplicate_usernames_partial(self, db_session):
        ...
        existing = [
            Account(username=f"existing_{i}", password="pass", status=AccountStatus.fresh)
            for i in range(5)
        ]
        ...
        new_accounts = [
            Account(username=f"existing_{i}", password="pass", status=AccountStatus.fresh)  # duplicate
            for i in range(3)
        ]
        new_accounts.extend([
            Account(username=f"new_{i}", password="pass", status=AccountStatus.fresh)  # new
            for i in range(7)
        ])
        ...
        for acc in new_accounts:
            existing_acc = db_session.query(Account).filter(Account.username == acc.username).first()
            if existing_acc:
...
class TestAccountRetrieval:
    ...
    def populated_accounts(self, db_session):
        ...
        accounts = [
            # 10 fresh accounts
            *(Account(username=f"fresh_{i}", password="pass", status=AccountStatus.fresh, account_type=AccountType.upvoter) for i in range(10)),
            # 20 logged_in accounts
            *(Account(username=f"logged_{i}", password="pass", status=AccountStatus.logged_in, account_type=AccountType.upvoter) for i in range(20)),
            # 5 rate_limited accounts
            *(Account(username=f"ratelimit_{i}", password="pass", status=AccountStatus.rate_limited, account_type=AccountType.upvoter) for i in range(5)),
            # 5 dead accounts
            *(Account(username=f"dead_{i}", password="pass", status=AccountStatus.dead, account_type=AccountType.upvoter) for i in range(5)),
            # 10 upvoter type
            # 10 both type
            *(Account(username=f"main_{i}", password="pass", status=AccountStatus.logged_in, account_type=AccountType.main) for i in range(10)),
        ]
    ...
    def test_get_accounts_filtered_by_status(self, db_session, populated_accounts):
        ...
        # Filter by logged_in
        logged_in = db_session.query(Account).filter(Account.status == AccountStatus.logged_in).all()
        assert len(logged_in) == 30  # 20 upvoter + 10 main
        
        # Filter by dead
        dead = db_session.query(Account).filter(Account.status == AccountStatus.dead).all()
        assert len(dead) == 5
        
        # Filter by rate_limited
        rate_limited = db_session.query(Account).filter(Account.status == AccountStatus.rate_limited).all()
    ...
    def test_get_accounts_filtered_by_type(self, db_session, populated_accounts):
        ...
        upvoters = db_session.query(Account).filter(Account.account_type == AccountType.upvoter).all()
        assert len(upvoters) == 40
        
        mains = db_session.query(Account).filter(Account.account_type == AccountType.main).all()
        assert len(mains) == 10
    ...
    def test_search_accounts_by_username(self, db_session, populated_accounts):
        ...
        # Search for "fresh"
        fresh = db_session.query(Account).filter(Account.username.contains("fresh")).all()
        assert len(fresh) == 10
        
        # Search for "logged"
        logged = db_session.query(Account).filter(Account.username.contains("logged")).all()
...
class TestAccountDeletion:
    ...
    def test_batch_delete_by_ids(self, db_session, accounts_to_delete):
        ...
        db_session.query(Account).filter(Account.id.in_(ids_to_delete)).delete(synchronize_session=False)
        db_session.commit()
    ...
    def test_batch_delete_by_filters(self, db_session, accounts_to_delete):
        ...
        db_session.query(Account).filter(Account.username.contains("delete_1")).delete(synchronize_session=False)
        db_session.commit()
        
        remaining = db_session.query(Account).count()
        assert remaining == 9  # 20 - 11 (1,10-19)
        
    def test_delete_account_with_session_file(self, db_session, fresh_account, tmp_path):
        ...
        assert db_session.query(fresh_account.__class__).filter_by(id=fresh_account.id).first() is None
