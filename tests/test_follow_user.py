"""
Tests for follow_user action - direct action testing without QueueProcessor.
"""
import time
import logging
import pytest
...
def get_mock_snapshots(scenario=None):
    ...
    if scenario == "banned":
        ...
        pre_click_base += '<button "Follow" [e12345]>Follow</button></body></html>'
...
class TestFollowUserDirect:
    """Tests for follow_user action - direct execution."""

    def test_follow_success(self, db_session):
        ...
        action = get_action_class("follow_user")(camofox)
    ...
    def test_follow_scenario_banned(self, db_session):
    ...
    def test_follow_scenario_suspended(self, db_session):
    ...
    def test_follow_scenario_locked(self, db_session):
        """Test follow with locked scenario (popup after click)."""
        from app.services.queue_actions import get_action_class

        logger.info(f"[TEST] Starting test_follow_scenario_locked")
        ...
        logger.info(f"[TEST] ✅ PASSED: test_follow_scenario_locked")
