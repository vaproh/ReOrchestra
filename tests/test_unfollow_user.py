"""
Tests for unfollow_user action - direct action testing without QueueProcessor.
"""
import time
import logging
import pytest
...
def get_mock_snapshots(scenario=None):
    ...
    <body>
    <div class="header">Reddit Header</div>
    <button "Follow" [e12345]>Follow</button>
    </body></html>
    '''
    post_click_locked = '''
    <html>
    <body>
    <div class="header">Reddit Header</div>
    <button "Following" [e12345]>Following</button>
    <div class="modal">
    ...
    if scenario == "banned":
        ...
        <button "Unfollow" [e12345]>Unfollow</button>
        '''
        ...
        pre_click_base += '<button "Unfollow" [e12345]>Unfollow</button></body></html>'
...
class TestUnfollowUserDirect:
    """Tests for unfollow_user action - direct execution."""

    def test_unfollow_success(self, db_session):
        ...
        action = get_action_class("unfollow_user")(camofox)
        ...
        result = action.execute(mock_worker, "http://test/user/test")
    ...
    def test_unfollow_scenario_banned(self, db_session):
        ...
        result = action.execute(mock_worker, "http://test/user/test?scenario=banned")
    ...
    def test_unfollow_scenario_suspended(self, db_session):
        ...
        result = action.execute(mock_worker, "http://test/user/test?scenario=suspended")
    ...
    def test_unfollow_scenario_locked(self, db_session):
        """Test unfollow with locked scenario (popup after click)."""
        from app.services.queue_actions import get_action_class

        logger.info(f"[TEST] Starting test_unfollow_scenario_locked")
        ...
        result = action.execute(mock_worker, "http://test/user/test?scenario=locked")
        ...
        logger.info(f"[TEST] ✅ PASSED: test_unfollow_scenario_locked")
